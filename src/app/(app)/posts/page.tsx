...
import Image from 'next/image'; // Keep Image if CoverSwitch or other parts use it, or if TimelinePostCard expects image objects
import Link from 'next/link'; // Keep Link for similar reasons
import CoverSwitch from './CoverSwitch';
import TimelinePostCard from './TimelinePostCard'; // Added import
...
// Removed PostCard function definition
...
export default function Posts() {
	...
	return (
		<Container className="min-h-[50vh] mt-16">
			...
			{/* Adjusted layout for timeline: centered, max-width for readability */}
			<div className="w-full max-w-3xl mx-auto">
				{sortedPosts.map((post, idx) => (
					<TimelinePostCard showCover={false} key={idx} post={post} />
				))}
			</div>
		</Container>
	);
